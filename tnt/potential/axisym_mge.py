...
oblate/prolate case. Deprojection happens once, in `_build` -- not lazily
inside `to_galax()` -- so an invalid inclination
(`tnt.mge.MGEDeprojectionError`), or an MGE with nonzero `PA_twist`
...
# The single viewing angle an axisymmetric MGE is deprojected under
# (`tnt.mge.AbstractMGE.deproject_axisymmetric`), in place of the triaxial
# module's `theta`/`phi`/`psi` -- required regardless of light vs. mass.
...
class AxisymmetricLightMGEPotential(AbstractPotentialComponent):
    ...
    and stores the result as `deprojected`; `to_galax` sums one
    `galax.potential.AxisymmetricGaussianPotential` per Gaussian component
    from it (see `_galax_potential_from_axisym_deprojected`). `inclination`
    ...
    def to_galax(
        ...
        return _galax_potential_from_axisym_deprojected(self.deprojected, unit_system)
...
class AxisymmetricMassMGEPotential(AbstractPotentialComponent):
    ...
    otherwise follow the same path as `AxisymmetricLightMGEPotential` -- see
    its docstring.
    """
...
def _galax_potential_from_axisym_deprojected(
    deprojected: Deprojected3DMGE,
    unit_system: AbstractUnitSystem,
) -> galax.potential.AbstractPotential:
    """Sum one `galax.potential.AxisymmetricGaussianPotential` per Gaussian component.

    An axisymmetric deprojection (`tnt.mge.AbstractMGE.deproject_axisymmetric`,
