...
Preparation rejects duplicate YAML keys and, for schemas owned by the
preparation layer, unknown fields, missing required fields, incorrect value
types, unsupported enumerated values, and inconsistent tagged thresholds. It
also checks data-only numerical constraints, including
